...
system_prompt = "You are a helpful assistant that extracts chapter titles from textbook PDFs."
            ...
            try:
                with open(pdf_path, 'rb') as f:
                    ...
                    if len(reader.pages) > 0:
                        ...
                        Return ONLY the Chapter Title in a clean format (e.g. "Baaj aur Saanp (बाज और साँप)", "Light", "Hindi Grammar"). Do not include any other text or quotes.

                        Text:
                        {pdf_text}
                        """
                        
                        new_title = generate_with_llm(system_prompt, user_prompt, 0.1, 50, 4096).strip()
